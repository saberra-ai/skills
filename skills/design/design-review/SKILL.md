---
name: design-review
description: Audit an existing web UI for near-native quality — run the mechanical checks (axe, Core Web Vitals, responsiveness/reflow at multiple viewports, motion/focus lints, AI-slop greps), then a judgment pass (hierarchy, restraint, feel), rank by severity, and fix forward with before/after proof. Use when an interface already exists and you want to find and fix what makes it feel AI-generated, web-laggy, or broken on mobile, rather than rebuild it.
---
...
2. **Run the mechanical pass — objective findings first:**
   ...
   - **Responsiveness**: run the pass at **≥3 widths** (≈320 / 768 / 1280px) + both orientations. Assert no horizontal scroll at **320px** ([WCAG 1.4.10 Reflow](https://www.w3.org/WAI/WCAG22/Understanding/reflow.html)), text to **200%** without clipping ([1.4.4](https://www.w3.org/WAI/WCAG22/Understanding/resize-text.html)), no orientation lock ([1.3.4](https://www.w3.org/WAI/WCAG22/Understanding/orientation.html)); grep `100vh` (should be `dvh`), `vw`-locked font-size, full-bleed without `env(safe-area-inset)`.
...
springs not easing · `:focus-visible` + reduced-motion honored · **reflow@320px no h-scroll · text@200% ·
no orientation lock**. Judgment: hierarchy, restraint, feel.
...
- Audited: <N findings — a11y / perf(CWV) / motion / responsive(reflow@320) / slop — each REAL (with measurement) or CLEARED>
- Fixed: <M fixed forward + re-measured; K cleared with reason>
- Artifact: <before/after — CWV numbers + screenshots per breakpoint (320/768/1280)>
